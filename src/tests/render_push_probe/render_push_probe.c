...
// buffer fetch works: the fragment colour now arrives via a push constant
// instead of being hardcoded in the shader.
//
// tests/driver_pipeline_probe already proved push constants work for
// compute. This is untested territory anyway - the compute path and the
// graphics path go through different parts of this driver (no IDVS, no
// tiler, no fragment stage on the compute side), so nothing about that
// result implies push constants reach a fragment shader correctly.
//
// The readback check is deliberately against the SPECIFIC value pushed at
// record time, not a hardcoded expectation living separately in this file -
// so a fragment shader that silently ignores the push constant and falls
// back to some other value (stale register, zero-initialised memory, a
// leftover from a previous test) would show up as a mismatch rather than
// an accidental pass. The pushed colour is deliberately not the same as
// any earlier probe's hardcoded magenta, for the same reason.
//
// Same vertex shader as render_vbo_probe (push.vert is byte-identical to
// vbo.vert - vertex fetch is already proven, unchanged here), same 16x16
// target, same partial-coverage triangle, same safety convention as every
// render_*/alias_cs probe in this repo: unbuffered stdout, refuses to run
// without --i-know-it-hangs.
//
// Usage: render_push_probe <path-to-libvulkan_panfrost.so> --i-know-it-hangs
#include <dlfcn.h>
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
...
#include "push_frag_spv.h"
#include "push_vert_spv.h"
...
/* Deliberately not the magenta earlier probes used - a fragment shader
 * that ignored the push constant and fell back to a stale/cached value
 * from a previous test would be caught by this being different.
 */
static const float PUSHED_COLOR[4] = {0x66 / 255.0f, 0xcc / 255.0f,
                                      0x33 / 255.0f, 1.0f};
static const uint8_t PUSHED_RGBA[4] = {0x66, 0xcc, 0x33, 0xff};
...
main(int argc, char **argv)
{
   ...
   if (argc < 3 || strcmp(argv[2], "--i-know-it-hangs") != 0) {
      fprintf(stderr,
              ...
              "proved, but the fragment colour now comes from a push\n"
              "constant instead of being hardcoded - one new variable: push\n"
              "constant plumbing through the graphics pipeline (compute\n"
              "already works, per driver_pipeline_probe, but the graphics\n"
              "path is different code and untested). tests/alias_cs_probe\n"
   ...
   VkApplicationInfo app = {
      ...
      .pApplicationName = "panvk-kbase-render-push-probe",
...
#define GDPA(name) (PFN_##name) gdpa(device, #name)
   ...
   /* -------------------------------------------------------- vertex buffer */
   printf("\n=== vertex buffer (same as render_vbo_probe) ===\n");
   ...
   /* --------------------------------------------------------- pipeline */
   printf("\n=== pipeline: fragment colour from a push constant ===\n");

   VkShaderModuleCreateInfo vs_smci = {
      .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
      .codeSize = sizeof(render_push_probe_vert),
      .pCode = render_push_probe_vert,
   };
   ...
   VkShaderModuleCreateInfo fs_smci = {
      .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
      .codeSize = sizeof(render_push_probe_frag),
      .pCode = render_push_probe_frag,
   };
   ...
   /* The actual thing under test: a real push constant range, where
    * render_triangle_probe/render_vbo_probe used an empty pipeline layout.
    */
   ...
   check(r == VK_SUCCESS, "vkCreatePipelineLayout (with push constant range)");
   ...
   /* ------------------------------------------------------------ record */
   printf("\n=== record: push a colour, then draw ===\n");
   ...
   cmd_push(cmdbuf, layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0,
            sizeof(PUSHED_COLOR), PUSHED_COLOR);
   printf("  vkCmdPushConstants recorded: rgba = %02x%02x%02x%02x\n",
          PUSHED_RGBA[0], PUSHED_RGBA[1], PUSHED_RGBA[2], PUSHED_RGBA[3]);
   ...
   if (r == VK_SUCCESS) {
      ...
      for (uint32_t px = 0; px < IMG_W * IMG_H; px++) {
         ...
         if (memcmp(p, CLEAR_RGBA, BYTES_PER_PIXEL) == 0)
            clear_px++;
         else if (memcmp(p, PUSHED_RGBA, BYTES_PER_PIXEL) == 0)
            tri_px++;
         else {
            if (!other_px)
               first_other = px;
      ...
      printf("  %u clear-colour, %u pushed-colour, %u other (of %u total)\n",
             clear_px, tri_px, other_px, IMG_W * IMG_H);
      printf("  render_triangle_probe/render_vbo_probe got 190/66/0 on the "
             "same geometry - %s\n",
      ...
      check(tri_px > 0,
            "some pixels hold the pushed colour (push constant reached "
            "the fragment shader)");
   ...
   printf("\n=== %d failure(s) ===\n", failures);
   if (failures == 0)
      printf("\n=> Push constants reach a graphics-pipeline fragment "
             "shader correctly on kbase.\n");
   return failures ? 1 : 0;
}
