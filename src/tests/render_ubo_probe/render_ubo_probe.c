// One variable changed from tests/render_push_probe, which proved push
// constants reach a graphics-stage fragment shader: the colour now comes
// from a uniform buffer through a real descriptor set instead of a push
// constant.
//
// Descriptor sets are the last basic plumbing mechanism this port had not
// exercised in a graphics pipeline. Everything a texture-sampling or
// transform-matrix shader needs beyond this is a variation on the same
// binding machinery - a combined image sampler instead of a uniform
// buffer, more bindings, more sets - not a new mechanism, so this is the
// gating unknown for "does anything resembling a real shader work."
//
// Same cross-check discipline as the push-constant probe: the readback is
// checked against the SPECIFIC value written into the UBO at record time
// (deliberately a third distinct colour, matching neither the push-constant
// probe's value nor the earlier hardcoded magenta), not a hardcoded
// expectation living separately in this file - so a descriptor set that
// silently resolves to garbage, a stale binding, or the wrong buffer would
// show up as a mismatch rather than an accidental pass.
//
...
//
// Usage: render_ubo_probe <path-to-libvulkan_panfrost.so> --i-know-it-hangs
#include <dlfcn.h>
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
...
#include "ubo_frag_spv.h"
#include "ubo_vert_spv.h"
...
/* A third distinct colour - not the push-constant probe's 0x66cc33, not
 * the earlier hardcoded magenta. A descriptor set silently resolving to
 * either of those (a stale binding from a previous test, or the shader
 * ignoring the UBO and falling back to some other constant) would be
 * caught by this being different from both.
 */
static const float UBO_COLOR[4] = {0x33 / 255.0f, 0x99 / 255.0f, 0xcc / 255.0f,
                                   1.0f};
static const uint8_t UBO_RGBA[4] = {0x33, 0x99, 0xcc, 0xff};
...
main(int argc, char **argv)
{
   ...
   if (argc < 3 || strcmp(argv[2], "--i-know-it-hangs") != 0) {
      fprintf(stderr,
              "This draws the same triangle tests/render_push_probe already\n"
              "proved, but the fragment colour now comes from a uniform\n"
              "buffer through a real descriptor set instead of a push\n"
              "constant - one new variable: descriptor set allocation,\n"
              "update, and binding in a graphics pipeline. Nothing about\n"
              "that path has run on this device before. tests/alias_cs_probe\n"
   ...
   VkApplicationInfo app = {
      ...
      .pApplicationName = "panvk-kbase-render-ubo-probe",
...
#define GDPA(name) (PFN_##name) gdpa(device, #name)
   ...
   /* ------------------------------------------------------------- UBO */
   printf("\n=== uniform buffer: the colour, via a real descriptor set ===\n");
   ...
   r = create_buffer(device, &ubo_bci, NULL, &ubo);
   check(r == VK_SUCCESS, "vkCreateBuffer (UBO)");

   VkMemoryRequirements ubo_reqs;
   get_buf_reqs(device, ubo, &ubo_reqs);
   uint32_t ubo_type =
      find_memory_type(&mem_props, ubo_reqs.memoryTypeBits, host_want);
   check(ubo_type != UINT32_MAX, "host-visible memory type found for UBO");
   if (ubo_type == UINT32_MAX)
      return 1;

   VkMemoryAllocateInfo ubo_mai = {
      .sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
      .allocationSize = ubo_reqs.size,
      .memoryTypeIndex = ubo_type,
   };
   VkDeviceMemory ubo_memory = VK_NULL_HANDLE;
   r = alloc_mem(device, &ubo_mai, NULL, &ubo_memory);
   check(r == VK_SUCCESS, "vkAllocateMemory (UBO)");
   r = bind_buf_mem(device, ubo, ubo_memory, 0);
   check(r == VK_SUCCESS, "vkBindBufferMemory (UBO)");

   void *ubo_mapped = NULL;
   r = map_mem(device, ubo_memory, 0, VK_WHOLE_SIZE, 0, &ubo_mapped);
   check(r == VK_SUCCESS, "vkMapMemory (UBO)");
   if (r != VK_SUCCESS)
      return 1;
   memcpy(ubo_mapped, UBO_COLOR, sizeof(UBO_COLOR));
   printf("  wrote colour %02x%02x%02x%02x into the UBO\n", UBO_RGBA[0],
          UBO_RGBA[1], UBO_RGBA[2], UBO_RGBA[3]);
   ...
   /* --------------------------------------------------- descriptor set */
   printf("\n=== descriptor set: one UBO binding ===\n");
   ...
   printf("  vkUpdateDescriptorSets returned - set points at the UBO\n");
   ...
   VkShaderModuleCreateInfo vs_smci = {
      ...
      .codeSize = sizeof(render_ubo_probe_vert),
      .pCode = render_ubo_probe_vert,
   };
   ...
   VkShaderModuleCreateInfo fs_smci = {
      .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
      .codeSize = sizeof(render_ubo_probe_frag),
      .pCode = render_ubo_probe_frag,
   };
   ...
   /* The actual thing under test: a pipeline layout built from a real
    * descriptor set layout, where every earlier probe used an empty one.
    */
   ...
   check(r == VK_SUCCESS, "vkCreatePipelineLayout (with descriptor set "
                          "layout)");
   ...
   /* ------------------------------------------------------------ record */
   printf("\n=== record: bind the descriptor set, then draw ===\n");
   ...
   if (r == VK_SUCCESS) {
      ...
      for (uint32_t px = 0; px < IMG_W * IMG_H; px++) {
         ...
         if (memcmp(p, CLEAR_RGBA, BYTES_PER_PIXEL) == 0)
            clear_px++;
         else if (memcmp(p, UBO_RGBA, BYTES_PER_PIXEL) == 0)
            tri_px++;
         else {
            if (!other_px)
               first_other = px;
      ...
      printf("  %u clear-colour, %u UBO-colour, %u other (of %u total)\n",
      ...
      check(tri_px > 0,
            "some pixels hold the UBO colour (descriptor set resolved "
            "to the right buffer)");
   ...
   destroy_buffer(device, ubo, NULL);
   free_mem(device, ubo_memory, NULL);
   destroy_buffer(device, vbo, NULL);
   free_mem(device, vbo_memory, NULL);

   printf("\n=== %d failure(s) ===\n", failures);
   if (failures == 0)
      printf("\n=> Descriptor sets work in a graphics pipeline on kbase:\n"
             "   allocation, update, and binding all resolved correctly.\n");
   return failures ? 1 : 0;
}
