// One variable changed from tests/render_triangle_probe, which proved a
// draw with a shader-hardcoded triangle works: this triangle's positions
// come from a real bound VkBuffer, fetched through actual vertex input
// state, instead of gl_VertexIndex into a hardcoded array.
//
// Almost every real Vulkan application uses vertex buffers - the hardcoded-
// position trick render_triangle_probe used is a rare fullscreen-triangle
// idiom, not normal usage. So this closes a real gap: vertex attribute
// fetch is untested until this passes, no matter how many hardcoded-
// position draws came before it.
//
// Same 16x16 target, same partial-coverage triangle, same magenta-on-clear
// readback check as render_triangle_probe - everything except the vertex
// input path is unchanged on purpose, so a failure here points at vertex
// buffer binding/fetch specifically rather than at rendering in general.
//
// Same safety convention as every render_*/alias_cs probe in this repo:
// unbuffered stdout, refuses to run without --i-know-it-hangs. This is a
// smaller increment than render_triangle_probe was over render_clear_probe
// - reusing an already-proven draw/pipeline shape, changing one input path
// - but it is still new hardware ground (vertex buffer DMA/fetch has not
// run on this device before either) and is gated accordingly.
//
// Usage: render_vbo_probe <path-to-libvulkan_panfrost.so> --i-know-it-hangs
#include <dlfcn.h>
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
...
/* Identical geometry to render_triangle_probe's hardcoded array, now living
 * in a real buffer instead. Keeping the numbers the same means a passing
 * result should reproduce that probe's exact pixel counts (190/66/0), which
 * is an extra cross-check: a different split would mean something about
 * vertex fetch is subtly wrong even though a naive pass/fail check might
 * not catch it.
 */
...
main(int argc, char **argv)
{
   ...
   if (argc < 3 || strcmp(argv[2], "--i-know-it-hangs") != 0) {
      fprintf(stderr,
              "This draws the same triangle tests/render_triangle_probe\n"
              "already proved, but fetched from a real vertex buffer\n"
              "instead of hardcoded shader positions - one new variable:\n"
              "vertex attribute fetch. Nothing about that path has run on\n"
              "this device before either. tests/alias_cs_probe hung the\n"
              "kbase context twice on adjacent GPU-fault territory -\n"
              "uninterruptible D state, needs a device reboot.\n"
              "\n"
              "Run tests/render_triangle_probe first if you have not\n"
              "already.\n"
              "\n"
   ...
   VkApplicationInfo app = {
      ...
      .pApplicationName = "panvk-kbase-render-vbo-probe",
...
#define GDPA(name) (PFN_##name) gdpa(device, #name)
   ...
   /* --------------------------------------------------------- pipeline */
   printf("\n=== pipeline: vertex fetch from a real buffer ===\n");
   ...
   /* The actual thing under test: a real binding + attribute description,
    * where render_triangle_probe used an empty VkPipelineVertexInputState.
    */
   ...
   /* ------------------------------------------------------------ record */
   printf("\n=== record: draw with a bound vertex buffer ===\n");
   ...
   printf("  vkCmdDraw(3, 1, 0, 0) recorded - positions from the bound "
          "buffer\n");
   ...
   if (r == VK_SUCCESS) {
      ...
      printf("  render_triangle_probe (hardcoded positions) got 190/66/0 "
             "on the same geometry - %s\n",
   ...
   printf("\n=== %d failure(s) ===\n", failures);
   if (failures == 0)
      printf("\n=> Vertex attribute fetch from a real bound buffer works on\n"
             "   kbase, matching the hardcoded-position result exactly.\n");
   return failures ? 1 : 0;
}
