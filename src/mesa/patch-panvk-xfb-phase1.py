...
patch_file(
    ...
    [
        ...
        (
            ...
            "   ids->vertex =\n"
            "      nir_isub(b, slot, nir_imul(b, ids->instance, num_vertices));\n"
            "\n"
            ...
            "\n"
            "   nir_push_if(b, nir_ine_imm(b, load_sysval(b, graphics, 64, xfb.index_buffer),\n"
            "                              0));\n"
            "   {\n"
            ...
            "         b, index_buf, nir_u2u64(b, nir_imul(b, ids->vertex, index_size)));\n"
            "\n"
            ...
            "   nir_pop_if(b, NULL);\n"
            "\n"
            "   ids->attrib = nir_if_phi(b, from_index, ids->vertex);\n"
            "}\n"
            "\n"
            "static bool\n"
...
patch_file(
    ...
    [
        ...
        (
            ...
            "/* VK_EXT_transform_feedback: vertices per primitive for the current topology.\n"
            " *\n"
            " * The capture emits one captured vertex per *input* vertex, which matches\n"
            " * transform feedback semantics only for LIST topologies. A triangle strip of N\n"
            " * vertices assembles N-2 triangles and must capture 3*(N-2) vertices, with\n"
            " * vertices duplicated across primitives; this implementation would capture N.\n"
            " * Rather than silently writing wrong data, strips and fans are rejected here.\n"
            " * Supporting them needs the capture grid to be sized by assembled primitives\n"
            " * and the store slot to be derived from the primitive/vertex pair, which is\n"
            " * the same machinery indirect draws will need.\n"
            " *\n"
            " * Restricting to lists also keeps the primitive count the XFB query reports a\n"
            " * plain divide, with no gallium u_prim.h dependency in the CSF path.\n"
            " */\n"
            "static uint32_t\n"
            ...
            "          \"pass\");\n"
            "\n"
            "   uint32_t vpp = xfb_verts_per_prim(cmdbuf);\n"
            "\n"
            ...
            "      gfx->xfb.pending_draws[n].verts_per_prim = vpp;\n"
...
patch_file(
    ...
    [
        (
            ...
            "                 uint32_t direct_instance_count, uint32_t verts_per_prim,\n"
            ...
            "\n"
            "   uint32_t generated_slots = vertex_count * instance_count;\n"
            "   uint32_t slots = generated_slots;\n"
            "\n"
            "   /* The capture shader recovers instance = slot / num_vertices, so for an\n"
            "    * indirect draw that sysval has to be patched in from here.\n"
            "    */\n"
            "   if (num_vertices_pu)\n"
            "      *num_vertices_pu = vertex_count;\n"
            "\n"
