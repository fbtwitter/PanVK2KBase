...
patch_file(
    ...
    [
        ...
        (
            ...
            "    * panvk_cmd_draw.h. Only drawCount == 1 is handled; more would need one\n"
            "    * capture per command so the write position advances between them.\n"
            "    */\n"
            "   if (cmdbuf->state.gfx.xfb.active) {\n"
            "      assert(drawCount == 1 &&\n"
            "             \"VK_EXT_transform_feedback: multi-draw indirect is not supported \"\n"
            "             \"yet\");\n"
            "      assert(!cmdbuf->vk.dynamic_graphics_state.ia.primitive_restart_enable &&\n"
            "             \"VK_EXT_transform_feedback: primitive restart with transform \"\n"
            "             \"feedback active is not supported yet\");\n"
            ...
            "      unsigned n = cmdbuf->state.gfx.xfb.pending_draw_count;\n"
            "      assert(n < ARRAY_SIZE(cmdbuf->state.gfx.xfb.pending_draws));\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].vertex_count = 0;\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].instance_count = 0;\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].vertex_base = 0;\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].index_buffer = 0;\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].index_size = 0;\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].query_ptr =\n"
            "         cmdbuf->state.gfx.xfb_query.ptr;\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].verts_per_prim =\n"
            "         xfb_verts_per_prim(cmdbuf);\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].indirect_buffer =\n"
            "         panvk_buffer_gpu_ptr(buffer, offset);\n"
            "      cmdbuf->state.gfx.xfb.pending_draw_count = n + 1;\n"
            "   }\n"
            "}\n",
        ),
        ...
        (
            ...
            "    * address is queued *unbiased* - firstIndex is in the indirect command, so\n"
            "    * panlib_xfb_setup() applies the bias. index_size being set alongside\n"
            "    * indirect_buffer is what marks this as an indexed indirect draw.\n"
            "    */\n"
            "   if (cmdbuf->state.gfx.xfb.active) {\n"
            "      const struct panvk_cmd_graphics_state *gfx = &cmdbuf->state.gfx;\n"
            "\n"
            "      assert(drawCount == 1 &&\n"
            "             \"VK_EXT_transform_feedback: multi-draw indexed indirect is not \"\n"
            "             \"supported yet\");\n"
            "      assert(!cmdbuf->vk.dynamic_graphics_state.ia.primitive_restart_enable &&\n"
            "             \"VK_EXT_transform_feedback: primitive restart with transform \"\n"
            "             \"feedback active is not supported yet\");\n"
            ...
            "      cmdbuf->state.gfx.xfb.pending_draws[n].instance_count = 0;\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].vertex_base = 0;\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].index_buffer = gfx->ib.dev_addr;\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].index_size = gfx->ib.index_size;\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].query_ptr =\n"
            "         cmdbuf->state.gfx.xfb_query.ptr;\n"
            "      cmdbuf->state.gfx.xfb.pending_draws[n].verts_per_prim =\n"
