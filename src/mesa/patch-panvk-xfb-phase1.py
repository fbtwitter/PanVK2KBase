...
patch_file(
    ...
    [
        (
            ...
            "      bool active;\n"
            "\n"
            "      /* Counter-buffer writebacks owed at CmdEndRendering, one entry per\n"
            "       * (buffer, End) pair. A render pass may contain several Begin/End\n"
            "       * pairs, each with its own write positions, and each End may name\n"
            "       * the same XFB buffer index as the last - so this cannot be a\n"
            "       * per-buffer array keyed by index, and each entry has to carry the\n"
            "       * write positions it belongs to rather than reading whichever pair's\n"
            "       * happen to be current at flush time.\n"
            "       *\n"
            "       * Same lifetime as pending_draws: cleared by\n"
            "       * cmd_flush_pending_xfb_captures(), freed on reset/destroy.\n"
            "       */\n"
            "      struct util_dynarray pending_counter_writes;\n"
            "\n"
            ...
            "       * friends are #if PAN_ARCH >= 13). Allocated fresh by every Begin;\n"
            "       * whoever needs it past the matching End (queued captures, pending\n"
            "       * counter writebacks) snapshots the address rather than reading it\n"
            "       * back from here, because the next Begin will have replaced it.\n"
            "       */\n"
        ...
        (
            ...
            "};\n"
            "\n"
            "/* One counter buffer owed a write at CmdEndRendering - see\n"
            " * xfb.pending_counter_writes.\n"
            " */\n"
            "struct panvk_xfb_counter_write {\n"
            "   /* The write positions of the Begin/End pair this writeback belongs\n"
            "    * to, not whichever pair is current when it is emitted.\n"
            "    */\n"
            "   uint64_t offsets;\n"
            "   uint64_t dev_addr;\n"
            "   uint32_t buf_idx;\n"
...
patch_file(
    ...
    [
        (
            ...
            "   util_dynarray_fini(&cmdbuf->state.gfx.xfb.pending_counter_writes);\n"
            "\n"
            "   memset(&cmdbuf->state, 0, sizeof(cmdbuf->state));\n"
            "   init_cs_builders(cmdbuf);",
        ),
