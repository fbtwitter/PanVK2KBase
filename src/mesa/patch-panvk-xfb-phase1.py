...
patch_file(
    ...
    [
        (
            ...
            "      bool active;\n"
            "\n"
            "      /* Counter-buffer reads and writes owed at CmdEndRendering, in the\n"
            "       * order they were recorded - see struct panvk_xfb_counter_op. Both\n"
            "       * directions are deferred, because a Begin that resumes from a\n"
            "       * counter buffer has to observe the End that wrote it, and that End\n"
            "       * is only final once the captures between them have run.\n"
            "       *\n"
            "       * Same lifetime as pending_draws: cleared by\n"
            "       * cmd_flush_pending_xfb_captures(), freed on reset/destroy.\n"
            "       */\n"
            "      struct util_dynarray pending_counter_ops;\n"
            "\n"
            "      /* Capture write position, one per bound XFB buffer, as a byte offset\n"
            "       * - the same unit a counter buffer uses, so seeding one from the\n"
            "       * other is a plain copy. Lives in GPU memory because\n"
            "       * panlib_xfb_setup() clamps against it: turning a byte offset into\n"
            "       * capture slots is a division, and the command stream cannot divide\n"
            "       * on PAN_ARCH 10 (cs_udiv32 and friends are #if PAN_ARCH >= 13).\n"
            "       * Allocated fresh by every Begin; whoever needs it past the matching\n"
            "       * End (queued captures, pending counter ops) snapshots the address\n"
            "       * rather than reading it back from here, because the next Begin will\n"
            "       * have replaced it.\n"
            "       */\n"
        ...
        (
            ...
            "\n"
            "/* Snapshots the state one queued capture needs and does its host-side\n"
            " * setup (push-uniform upload, descriptor and scratch allocations), so\n"
            " * that the dispatch emitted later at CmdEndRendering reads none of it\n"
            " * live. Called from each queueing site in csf/panvk_vX_cmd_draw.c right\n"
            " * after the pending draw is filled in; defined in\n"
            " * csf/panvk_vX_cmd_xfb.c. See struct panvk_xfb_pending_draw.\n"
            " */\n"
            "struct panvk_xfb_pending_draw;\n"
            "void panvk_per_arch(cmd_prepare_xfb_capture)(\n"
            "   struct panvk_cmd_buffer *cmdbuf, struct panvk_xfb_pending_draw *draw);\n",
        ),
        (
            ...
            "\n"
            "   /* Everything below is the state snapshot.\n"
            "    *\n"
            "    * The capture dispatches all run at CmdEndRendering, long after the\n"
            "    * draw that queued them was recorded, so anything the dispatch reads\n"
            "    * out of live command-buffer state would be the *last* draw's, not\n"
            "    * this one's. A render pass with several Begin/End pairs, or simply\n"
            "    * two draws with different push constants, then gives every capture\n"
            "    * the same (wrong) inputs. So the host-side half of the dispatch runs\n"
            "    * at record time instead - panvk_per_arch(cmd_prepare_xfb_capture)()\n"
            "    * in csf/panvk_vX_cmd_xfb.c - and leaves its results here. What is\n"
            "    * left for CmdEndRendering is pure command-stream emission, which\n"
            "    * genuinely does belong there (it needs the sync point flush_tiling()\n"
            "    * only signals then).\n"
            "    *\n"
            "    * false if the capture could not be prepared (no XFB shader variant,\n"
            "    * or an allocation failed - the error is already on the command\n"
            "    * buffer); the entry is then skipped at flush time.\n"
            "    */\n"
            "   bool prepared;\n"
            "\n"
            "   /* The XFB variant that was bound when this draw was recorded, along\n"
            "    * with the descriptor table it fetches attributes through. Both move\n"
            "    * with every vkCmdBindPipeline / vkCmdBindDescriptorSets.\n"
            "    */\n"
            "   const struct panvk_shader_variant *xfb_variant;\n"
            "   uint64_t res_table;\n"
            "\n"
            "   /* Uploaded from this draw's push constants and sysvals. */\n"
            "   uint64_t push_uniforms;\n"
            "\n"
            "   uint64_t spd;\n"
            "   uint64_t tsd;\n"
            "\n"
            "   /* panlib_xfb_setup() arguments resolved at record time. offsets is the\n"
            "    * write-position allocation of the Begin/End pair this draw is inside.\n"
            "    */\n"
            "   uint64_t offsets;\n"
            "   uint64_t descs;\n"
            "   uint32_t desc_count;\n"
            ...
            "   uint64_t slot_table;\n"
            "\n"
            "   /* Addresses of individual push-uniform slots the setup kernel patches\n"
            "    * in place, 0 when the shader does not read that sysval.\n"
            "    */\n"
            "   uint64_t first_vertex_pu;\n"
            "   uint64_t index_buffer_pu;\n"
            "   uint64_t num_vertices_pu;\n"
            "};\n"
            "\n"
            "enum panvk_xfb_counter_op_type {\n"
            "   /* Begin resuming from a counter buffer: counter -> write position. */\n"
            "   PANVK_XFB_COUNTER_SEED,\n"
            "   /* End publishing a counter buffer: write position -> counter. */\n"
            "   PANVK_XFB_COUNTER_WRITEBACK,\n"
            "};\n"
            "\n"
            "/* One counter-buffer transfer owed at CmdEndRendering, for one buffer -\n"
            " * see xfb.pending_counter_ops.\n"
            " *\n"
            " * Both directions are deferred, and that is the point. A render pass may\n"
            " * Begin/End several times, with each Begin resuming from the counter\n"
            " * buffer the previous End wrote (CTS calls this a backward dependency,\n"
            " * and it is what vkCmdDrawIndirectByteCountEXT does too). The write\n"
            " * positions those Ends publish are only final once the captures have\n"
            " * run, which is at CmdEndRendering - so if a Begin emitted its read when\n"
            " * it was recorded, the command stream would read the counter buffer\n"
            " * before anything had written it, and every pair would start from the\n"
            " * same place and overwrite the last.\n"
            " *\n"
            " * Deferring both and replaying them in record order, interleaved with the\n"
            " * captures, is what makes the sequence come out as the application wrote\n"
            " * it. draw_pos is what keeps that interleaving honest.\n"
            " */\n"
            "struct panvk_xfb_counter_op {\n"
            "   enum panvk_xfb_counter_op_type type;\n"
            "\n"
            "   /* The write positions of the Begin/End pair this op belongs to, not\n"
            "    * whichever pair is current when it is emitted.\n"
            "    */\n"
            "   uint64_t offsets;\n"
            "\n"
            "   /* The application's counter buffer, at the offset it supplied. */\n"
            "   uint64_t dev_addr;\n"
            "\n"
            "   uint32_t buf_idx;\n"
            "\n"
            "   /* How many captures had been queued when this op was recorded, i.e.\n"
            "    * the index in xfb.pending_draws this op sorts before. Ops are\n"
            "    * appended in record order and this never decreases, so replaying\n"
            "    * every op with draw_pos <= i before capture i reproduces the\n"
            "    * recorded order exactly.\n"
            "    */\n"
            "   uint32_t draw_pos;\n"
            "};\n"
            "\n"
...
patch_file(
    ...
    [
        ...
        (
            ...
            "\n"
            "   /* nir_lower_xfb_to_stores rewrote the shader's gl_VertexIndex into\n"
            "    * raw_vertex_id + raw_vertex_offset. Here raw_vertex_id is the capture\n"
            "    * slot within the instance, so the offset has to carry the difference\n"
            "    * back to the attribute-fetch index - which already accounts for the\n"
            "    * index buffer - plus the draw's vertex base. That base is the value\n"
            "    * GLOBAL_ATTRIBUTE_OFFSET is programmed with, i.e. what hardware\n"
            "    * attribute fetch adds on top of the index it is given.\n"
            "    *\n"
            "    * The sum is then exactly gl_VertexIndex for indexed and non-indexed\n"
            "    * draws alike, and expressing it this way means not having to\n"
            "    * pattern-match the iadd the pass emitted.\n"
            "    */\n"
            "   nir_def *raw_vertex_offset =\n"
            "      nir_iadd(&b, nir_isub(&b, ids.attrib, ids.vertex),\n"
            "               load_sysval(&b, graphics, 32, vs.first_vertex));\n"
            "\n"
...
patch_file(
    ...
    [
        (
            ...
            "         /* index_count_bound stays 0: it only sizes the primitive-restart\n"
            "          * slot table, and restart needs an index buffer this draw has not\n"
            "          * got.\n"
            "          */\n"
            "         d->query_ptr = cmdbuf->state.gfx.xfb_query.ptr;\n"
            "         d->xfb_topology = xfb_topology(cmdbuf);\n"
            "\n"
            "         panvk_per_arch(cmd_prepare_xfb_capture)(cmdbuf, d);\n"
            "      }\n"
            "   }\n"
            "}",
        ),
        ...
        (
            ...
            "                                            : 0xffu)\n"
            "               : 0;\n"
            "         /* Sizes the restart slot table. Exact here - a direct indexed\n"
            "          * draw consumes exactly indexCount indices.\n"
            "          */\n"
        ...
        (
            ...
            "                &cmdbuf->state.gfx.xfb.pending_counter_ops,\n"
            "                struct panvk_xfb_counter_op))\n"
...
patch_file(
    ...
    [
        (
            ...
            "                 uint32_t restart_index, global uint32_t *first_vertex_pu)\n"
            "{\n"
            ...
            "\n"
            "   /* The shader's gl_VertexIndex needs the draw's vertex base, which for an\n"
            "    * indirect draw lives in the command: firstVertex is word 2 of\n"
            "    * VkDrawIndirectCommand, vertexOffset word 3 of the indexed form. Same\n"
            "    * value GLOBAL_ATTRIBUTE_OFFSET is loaded with.\n"
            "    */\n"
            "   if (indirect && first_vertex_pu)\n"
            "      *first_vertex_pu = indirect[index_size ? 3 : 2];\n"
            "\n"
...
patch_file(
    ...
    [
        (
            ...
            "   util_dynarray_fini(&cmdbuf->state.gfx.xfb.pending_counter_ops);\n"
            "\n"
            "   memset(&cmdbuf->state, 0, sizeof(cmdbuf->state));\n"
            "   init_cs_builders(cmdbuf);",
        ),
